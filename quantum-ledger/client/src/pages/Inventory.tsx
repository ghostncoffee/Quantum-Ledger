...
import { fmtCurrency, findStarCitizenGame } from '@/lib/utils';
...
export function Inventory() {
  ...
  const [newForm, setNewForm] = useState({ item: '', quantity: '', unitCost: '', location: '' });
  ...
  const { data: inventory = [] } = useQuery({ queryKey: ['inventory', scGameId], queryFn: () => inventoryApi.list(scGameId ? { gameId: scGameId } : undefined) });
  const { data: committedBags = [] } = useQuery({
    queryKey: ['committed-ore', scGameId],
    queryFn: () => miningApi.getCommitted(scGameId ? Number(scGameId) : undefined),
  });
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <Button className="mt-2" size="sm" onClick={() => {
          if (!newForm.item) return;
          add.mutate({ gameId: scGameId, item: newForm.item, quantity: Number(newForm.quantity) || 0, unitCost: newForm.unitCost ? Number(newForm.unitCost) : undefined, location: newForm.location || undefined });
          setNewForm({ item: '', quantity: '', unitCost: '', location: '' });
        }}><Plus size={13} /> Add</Button>
      </Card>

      <Card className="p-0">
        <Table>
          <thead><tr><Th>Item</Th><Th>Quantity</Th><Th>Unit Cost</Th><Th>Total Value</Th><Th>Location</Th><Th /></tr></thead>
          <tbody>
