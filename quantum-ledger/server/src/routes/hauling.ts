...
async function syncJobStatus(jobId: number) {
  ...
  if (allDone) {
    ...
    // Fire-and-forget clan sync for delivered haul
    const job = await db.get('SELECT agreed_payout, bonus_payout, notes, pickup_location, delivery_location FROM hauling_jobs WHERE id = ?', [jobId]);
    if (job) {
      void syncSession({
        session_type: 'hauling',
        occurred_at: new Date().toISOString(),
        data: {
          agreed_payout: job.agreed_payout,
          bonus_payout: job.bonus_payout ?? 0,
          pickup_location: job.pickup_location ?? null,
          dropoff_location: job.delivery_location ?? null,
