...
function CraftingPanel({ runId, currency, gameId }: { runId: number; currency: string; gameId: number }) {
  ...
  return (
    <div className="space-y-4">
            ...
            return (
              <Card key={job.id}>
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  >
                    <ChevronRight size={13} className={`shrink-0 text-slate-500 transition-transform duration-150 ${expanded ? 'rotate-90' : ''}`} />
      ...
      {completed.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Completed ({completed.length})</h2>
          <Card className="p-0">
            <Table>
              <thead><tr><Th>Item</Th><Th>Qty</Th><Th>Input cost</Th><Th>Est. value</Th><Th>Margin</Th><Th /></tr></thead>
              <tbody>
                {completed.map((job: any) => {
                  ...
                  return (
                    <Tr key={job.id} className="opacity-70">
                      ...
                      <Td className={margin != null ? profitColor(margin) : 'text-slate-500'}>{margin != null ? fmtCurrency(margin, currency) : '—'}</Td>
                      <Td><Button variant="danger" size="sm" onClick={() => removeJob.mutate(job.id)}><Trash2 size={12} /></Button></Td>
                    </Tr>
                  );
                })}
              </tbody>
