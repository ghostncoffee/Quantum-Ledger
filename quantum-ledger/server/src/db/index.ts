...
CREATE TABLE IF NOT EXISTS vehicles (
  ...
  scu_capacity REAL NOT NULL DEFAULT 0,
  game_id INTEGER REFERENCES games(id),
  notes TEXT
);
...
export async function initDb() {
  ...
  const migrations = [
    ...
    'ALTER TABLE vehicles ADD COLUMN crew_min INTEGER NOT NULL DEFAULT 1',
    'ALTER TABLE vehicles ADD COLUMN crew_max INTEGER NOT NULL DEFAULT 1',
    'ALTER TABLE vehicles ADD COLUMN scu_capacity REAL NOT NULL DEFAULT 0',
    'ALTER TABLE vehicles ADD COLUMN crew_count INTEGER NOT NULL DEFAULT 1',
  ...
  try {
    await db.run('UPDATE vehicles SET crew_min = crew_count, crew_max = crew_count WHERE crew_count IS NOT NULL');
  } catch {
    // no-op if column missing or update fails
  }
  ...
  // Deduplicate existing blueprints before enforcing the unique index
  try {
    await db.run(`
      ...
      WHERE id NOT IN (
        SELECT MIN(id)
        FROM blueprints
        GROUP BY game_id, product_name, mission_guid, mission_trigger
      )
    `);
    await db.run('CREATE UNIQUE INDEX IF NOT EXISTS idx_blueprints_unique ON blueprints(game_id, product_name, mission_guid, mission_trigger)');
  } catch (err) {
    console.warn('[db] Could not create unique blueprint index during init:', err);
  }
