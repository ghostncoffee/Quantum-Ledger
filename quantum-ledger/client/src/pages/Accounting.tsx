...
function NewEntryModal({ open, onClose, defaultGameId }: { open: boolean; onClose: () => void; defaultGameId?: number }) {
  const qc = useQueryClient();
  const [form, setForm] = useState({ type: 'income', category: 'other', amount: '', description: '', date: new Date().toISOString().split('T')[0] });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add Ledger Entry">
      <form onSubmit={e => { e.preventDefault(); add.mutate({ gameId: defaultGameId, type: form.type, category: form.category, amount: Number(form.amount), description: form.description, date: form.date }); }} className="space-y-3">
...
export function Accounting() {
  ...
  const { data: ledger = [] } = useQuery({ queryKey: ['ledger', scGameId], queryFn: () => accountingApi.list(scGameId ? { gameId: scGameId } : undefined) });
  const { data: runReport = [] } = useQuery({ queryKey: ['runs-report', scGameId], queryFn: () => accountingApi.runsReport(scGameId ? { gameId: scGameId } : undefined) });
  ...
  const curr = scGame?.currency || 'UEC';
  ...
  return (
    <div className="space-y-4">
      ...
      <NewEntryModal open={newOpen} onClose={() => setNewOpen(false)} defaultGameId={scGameId} />
    </div>
  );
}
