...
import { syncSession } from '../lib/clanSync';
...
router.post('/bags/:id/commit', async (req, res) => {
  ...
  try {
    ...
    // Fire-and-forget clan sync
    const bag = await db.get(
      `SELECT mb.*, r.title AS run_title FROM mining_bags mb
       JOIN runs r ON mb.run_id = r.id WHERE mb.id = ?`,
      [req.params.id],
    );
    const lines = await db.all(
      'SELECT material, scu, quality FROM mining_ore_lines WHERE bag_id = ?',
      [req.params.id],
    );
    void syncSession({
      session_type: 'mining',
      ...
      data: {
        bag_label: bag?.label ?? null,
        committed_location: location ?? null,
        run_title: bag?.run_title ?? null,
        ore_lines: (lines as any[]).map((l: any) => ({
          material: l.material, scu: l.scu, quality: l.quality,
        })),
      },
    });
  } catch (e: unknown) { routeError(res, e); }
  ...
  try {
    ...
    if (status === 'done') {
      ...
      if (resolvedGameId) {
        ...
        // Fire-and-forget clan sync for refining completion
        const sessionRow = await db.get(
          'SELECT station, method, total_cost FROM refinery_sessions WHERE id = ?',
          [req.params.id],
        );
        void syncSession({
          session_type: 'refining',
          occurred_at: new Date().toISOString(),
          data: {
            station: sessionRow?.station ?? null,
            method: sessionRow?.method ?? null,
            total_cost: sessionRow?.total_cost ?? null,
            outputs: (completedLines as any[]).map((l: any) => ({
              material: l.output_material,
              quantity: l.output_quantity,
            })),
          },
        });
      }
