...
router.post('/:id/complete', async (req, res) => {
  ...
  try {
    ...
    // Fire-and-forget clan sync for completed contract
    const contract = await db.get(
      'SELECT type, client_name, agreed_payout, bonus_payout FROM contracts WHERE id = ?',
      [req.params.id],
    );
    if (contract) {
      void syncSession({
        ...
        data: {
          ...
          agreed_payout: contract.agreed_payout,
          bonus_payout: contract.bonus_payout ?? 0,
        },
      });
    }
  } catch (e: unknown) { routeError(res, e); }
