# start.ps1 — Quantum Ledger dev launcher
# Usage: .\start.ps1
...
Write-Host "`nQuantum Ledger — dev start`n" -ForegroundColor Cyan
...
if (-not $alreadyUp) {
    ...
    $tmp = Join-Path $env:TEMP "quantum-ledger-server.ps1"
    @"
Write-Host 'Quantum Ledger — API Server' -ForegroundColor Cyan
node '$tsx' '$entry'
