...
import {
  ...
  Package, MapPin, Users, Car, BookOpen, ChevronDown, Gamepad2,
  FlaskConical, Recycle, Zap, List,
} from 'lucide-react';
...
const GITHUB_URL = 'https://github.com/ghostncoffee/Quantum-Ledger';

const RUN_PATHS = ['/mining', '/refining', '/salvaging', '/trading', '/crafting', '/contracts', '/runs'];
...
function Divider() {
  return <div className="my-2 border-t border-[#1e2d4f]/50" />;
}

export function Sidebar() {
  const location = useLocation();
  const isRunActive = RUN_PATHS.some(p =>
    location.pathname === p || location.pathname.startsWith(p + '/'),
  );
  const [runsOpen, setRunsOpen] = useState(isRunActive);

  useEffect(() => {
    if (isRunActive) setRunsOpen(true);
  }, [isRunActive]);
  ...
  return (
    <aside className="flex h-screen w-56 flex-col border-r border-[#1e2d4f] bg-[#0a0e1a]">
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-[#1e2d4f]">
        <img src={logoSvg} alt="Quantum Ledger" className="h-8 w-8 rounded-lg shrink-0" />
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2">
        ...
        {/* All Runs — collapsible */}
        <button
          onClick={() => setRunsOpen(o => !o)}
          className={cn(
            'w-full flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors mb-0.5',
            isRunActive
              ? 'bg-blue-600/20 text-blue-300 border border-blue-700/40'
              : 'text-slate-400 hover:bg-[#141c35] hover:text-slate-200',
          )}
        >
          <List size={15} />
          <span className="flex-1 text-left">All Runs</span>
          <ChevronDown
            size={13}
            className={cn('transition-transform duration-200', runsOpen && 'rotate-180')}
          />
        </button>

        {runsOpen && (
          <div className="ml-3 pl-2 border-l border-[#1e2d4f]/70 mb-0.5">
            {RUN_ITEMS.map(item => (
        ...
        <NavItem to="/blueprints" icon={Zap}      label="Blueprints" />

        <Divider />

        <NavItem to="/crew"     icon={Users} label="Crew"     />
        <NavItem to="/vehicles" icon={Car}   label="Vehicles" />

        <Divider />

        <NavItem to="/settings" icon={Gamepad2} label="Settings" />
      </nav>
