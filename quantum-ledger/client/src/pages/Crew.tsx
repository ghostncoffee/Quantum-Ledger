...
import { crewApi, gamesApi, clanApi } from '@/lib/api';
import { Card, CardHeader, CardTitle } from '@/components/ui/Card';
import { Button } from '@/components/ui/Button';
import { Modal } from '@/components/ui/Modal';
import { Table, Th, Td, Tr } from '@/components/ui/Table';
import { Plus, Trash2, Users, Star, ServerIcon } from 'lucide-react';
import { findStarCitizenGame } from '@/lib/utils';

function NewCrewModal({ open, onClose, defaultGameId }: { open: boolean; onClose: () => void; defaultGameId?: number }) {
  const qc = useQueryClient();
  const [form, setForm] = useState({ name: '', gameHandle: '', notes: '' });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add Crew Member">
      <form onSubmit={e => { e.preventDefault(); add.mutate({ name: form.name, gameHandle: form.gameHandle || undefined, gameId: defaultGameId, notes: form.notes || undefined }); }} className="space-y-3">
        <div>
...
export function Crew() {
  ...
  const { data: crew = [] } = useQuery({ queryKey: ['crew', scGameId], queryFn: () => crewApi.list(scGameId) });
  const { data: clanMembers = [] } = useQuery({
    queryKey: ['clan-members'],
    queryFn: clanApi.members,
    staleTime: 5 * 60 * 1000,
    retry: false,
  });

  // Handles already registered locally (by game_handle or name, case-insensitive)
  const localHandles = new Set([
    ...(crew as any[]).map(m => String(m.game_handle || '').toLowerCase()),
    ...(crew as any[]).map(m => String(m.name || '').toLowerCase()),
  ].filter(Boolean));
  const unregisteredClanMembers = (clanMembers as any[]).filter(
    m => m.username && !localHandles.has(String(m.username).toLowerCase())
  );
  ...
  const addFromClan = useMutation({
    mutationFn: (username: string) => crewApi.create({ name: username, gameHandle: username, gameId: scGameId }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['crew'] }),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-0">
        <Table>
          <thead><tr><Th>Name</Th><Th>Handle</Th><Th>Notes</Th><Th /></tr></thead>
          <tbody>
            {sorted.length === 0 ? (
      ...
      {unregisteredClanMembers.length > 0 && (
        <Card className="p-0">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <ServerIcon size={15} className="text-slate-400" />
              Available from clan server
            </CardTitle>
          </CardHeader>
          <Table>
            <thead>
            ...
            <tbody>
              {unregisteredClanMembers.map((m: any) => (
                <Tr key={m.username}>
                  <Td className="font-medium text-slate-300">{m.username}</Td>
                  <Td className="text-slate-500 text-xs">{m.last_seen ? new Date(m.last_seen).toLocaleDateString() : '—'}</Td>
                  <Td>
                    <Button
                      size="sm"
                      onClick={() => addFromClan.mutate(m.username)}
                      disabled={addFromClan.isPending}
                    >
      ...
      <NewCrewModal open={newOpen} onClose={() => setNewOpen(false)} defaultGameId={scGameId} />
    </div>
  );
}
